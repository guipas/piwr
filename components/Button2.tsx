...
import { Labels } from '../interfaces/config';
...
export const Button2 = (props: IButtonProps) => {
  ...
  const confirmText = props.label.confirm || 'Click again to confirm';
  ...
  const check = async () => {
    ...
    try {
      ...
      setStatus({
        text: data?.text || props.label?.success || '',
        state: data?.state || 'success',
      });
      await minWait;
    } catch(e: any) {
      setStatus({
        text: e.response?.data?.text || props.label?.error || e.toString(),
  ...
  const doAction = async () => {
    ...
    try {
      ...
      setStatus({
        text: props.label?.success, state: 'success',
        ...data,
      });
    } catch (e: any) {
      setStatus({
